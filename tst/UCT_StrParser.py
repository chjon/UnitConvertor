...
def test_aggregation(verbose = False):
	...
	# Test aggregating with multiple terms
	test_result += aggregation_expect(["1", "cm", "+", "1", "m"], [("1", ["cm"]), "+", ("1", ["m"])], verbose)
	test_result += aggregation_expect(["1", "cm", "-", "1", "m"], [("1", ["cm"]), "-", ("1", ["m"])], verbose)
	test_result += aggregation_expect(["1", "cm", "*", "1", "m"], [("1", ["cm"]), "*", ("1", ["m"])], verbose)
	test_result += aggregation_expect(["1", "cm", "/", "1", "m"], [("1", ["cm"]), "/", ("1", ["m"])], verbose)
	test_result += aggregation_expect(["1", "cm", "^", "1", "m"], [("1", ["cm", "^", "1", "*", "m"])], verbose)
	test_result += aggregation_expect(["1", "cm", ":", "1", "m"], [("1", ["cm"]), ":", ("1", ["m"])], verbose)
	test_result += aggregation_expect(["cm", "^", "1", "+", "m"], [("1", ["cm", "^", "1"]), "+", ("1", ["m"])], verbose)
...
def test_parser(verbose = False):
	...
	test_result += parseExpr_expect([("+1", [])], "1", verbose)
	test_result += parseExpr_expect([("1", ["cm"]), ("1", ["m"]), ":"], "(1 cm : 1 m)", verbose)
	test_result += parseExpr_expect([("1", ["cm"]), ("1", ["m"]), "+"], "(1 cm + 1 m)", verbose)
	test_result += parseExpr_expect([("1", ["cm"]), ("1", ["m"]), "-"], "(1 cm - 1 m)", verbose)
	test_result += parseExpr_expect([("1", ["cm"]), ("1", ["m"]), "*"], "(1 cm * 1 m)", verbose)
	test_result += parseExpr_expect([("1", ["cm"]), ("1", ["m"]), "/"], "(1 cm / 1 m)", verbose)
	test_result += parseExpr_expect([("1", ["cm"]), ("1", []), "^"], "((1 cm)^(1))", verbose)
	test_result += parseExpr_expect([("1", ["cm"]), ("1", ["cm"]), "^"], "((1 cm)^(1 cm))", verbose)
